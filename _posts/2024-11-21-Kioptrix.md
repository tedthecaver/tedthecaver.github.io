...
- Kioptrix is a vulnerable machine that we will be attempting to gain root access to

Now going onto our kali machine we determine our ip with `ifconfig` and then perform `netdiscover -r x.y.z.0/24` which uses address resolution protocol to discover everything on the network and sweep everything on the subnet of 24.
