...
Now going onto our kali machine we determine our ip with `ifconfig` and then use the command `netdiscover -r x.y.z.0/24` which uses address resolution protocol to discover every ip address on the subnet of 24.
